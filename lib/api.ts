...
async function fetchApi<T>(endpoint: string, options?: RequestInit): Promise<ApiResponse<T>> {
  try {
    ...
    if (!response.ok) {
      return { data: null, error: data.error || data.message || 'Request failed', success: false };
    }
...
export const authApi = {
  login: (email: string, password: string) =>
    fetchApi<{ user: { id: string; email: string; name: string; role: string; employeeId?: string }; token: string }>('/auth/login', {
      method: 'POST',
